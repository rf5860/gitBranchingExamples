...
const DEVELOP_COLOR = "#B18DE6";
const RELEASE_COLOR = "#53D3D5";
gitGraph.template.colors = [ FEATURE_COLOR, HOTFIX_COLOR, MASTER_COLOR, DEVELOP_COLOR, FEATURE_COLOR, FEATURE_COLOR ];
...
const feature_b = release.branch({ name: "feature/b", column: 6 });
...
// Finish 'hotfix/1.1' (Merge it to 'master', 'develop', and 'release/2.0')
hotfix.merge(release, { labelFont: DEFAULT_FONT, color: RELEASE_COLOR });
hotfix.merge(develop, { labelFont: DEFAULT_FONT, color: DEVELOP_COLOR });
hotfix.merge(master, { tag: "v1.1", color: MASTER_COLOR });
// Do more work on 'feature/b' then merge to 'release/2.0'
feature_b.commit()
    .commit()
    .merge(release);
// Finish 'release/2.0' (Merge it to 'master' and 'develop')
